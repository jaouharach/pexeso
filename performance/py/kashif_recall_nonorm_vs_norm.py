...
for pv in pexeso_versions:
    ...
    for sm in search_modes:
        for kv in kashif_versions:
            csv_files = {"kashif": (base_path / f"{kashif_versions[kv]}{sm}kashif_results.csv").resolve(), "pexeso": (base_path / f"{curr_pexeso_version}pexeso_results.csv").resolve()}
            ...
            rows.append({'tau' : pv, 'data' : kv, 'avg. precision': mean_precision, 'avg. recall': mean_recall})
...
g = sns.barplot(data=df, x='tau', y='avg. recall', hue='data', palette=sns.color_palette(flatui))
...
plt.savefig(f"{outdir}kashif_recall_nonorm_vs_norm.png")
plt.close()