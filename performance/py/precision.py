...
g = sns.barplot(data=df, x='tau', y='avg. precision', hue='search-mode', palette=sns.color_palette(flatui))

plt.subplots_adjust(bottom=0.15)
plt.subplots_adjust(left=0.15)
plt.title("Kashif avg. precision (100k tables, 4.9M vectors, k = 1.000)")
plt.xlabel(r'$\mathrm{tau\ (\%)}$', fontsize = 11)
plt.ylabel(r'$\mathrm{avg.\ precision}$', fontsize = 11)
# plt.legend(loc='best')
plt.xticks(fontsize = 11)
plt.yticks(fontsize = 11)
plt.ylim([0, 1])
plt.grid()  #just add this
plt.savefig(f"{outdir}kashif_avg_precision.png")
plt.close()